...
class MembersController extends SkinnyResource with ApplicationController {
  ...
  beforeAction() {
    ...
    set("positions", Position.findAll())
  }
  ...
  object validPosition extends ValidationRule {
    def name = "validPosition"
    def isValid(v: Any) = (isNumeric(v) && {
      Position.findById(v.toString.toLong).isDefined
    })
  }
